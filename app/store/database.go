...
func (db *database) init() error {
	...
	if err := db.ensureCurrentSettingsColumns(); err != nil {
		return fmt.Errorf("ensure current settings columns: %w", err)
	}
...
func (db *database) migrate() error {
	...
	for version < currentSchemaVersion {
		...
		case 15:
			...
			version = 16
		case 16:
			// add start_minimized column to settings table
...
// migrateV16ToV17 adds the start_minimized column to the settings table
func (db *database) migrateV16ToV17() error {
	if err := db.ensureStartMinimizedColumn(); err != nil {
		return err
	}
...
func (db *database) ensureCurrentSettingsColumns() error {
	return db.ensureStartMinimizedColumn()
}

func (db *database) ensureStartMinimizedColumn() error {
	_, err := db.conn.Exec(`ALTER TABLE settings ADD COLUMN start_minimized BOOLEAN NOT NULL DEFAULT 0`)
	if err != nil && !duplicateColumnError(err) {
		return fmt.Errorf("add start_minimized column: %w", err)
	}
...
func (db *database) getSettings() (Settings, error) {
	...
	err := db.conn.QueryRow(`
		SELECT expose, survey, browser, models, agent, tools, working_dir, context_length, turbo_enabled, websearch_enabled, selected_model, sidebar_open, last_home_view, start_minimized, think_enabled, think_level, auto_update_enabled
		FROM settings
	`).Scan(&s.Expose, &s.Survey, &s.Browser, &s.Models, &s.Agent, &s.Tools, &s.WorkingDir, &s.ContextLength, &s.TurboEnabled, &s.WebSearchEnabled, &s.SelectedModel, &s.SidebarOpen, &s.LastHomeView, &s.StartMinimized, &s.ThinkEnabled, &s.ThinkLevel, &s.AutoUpdateEnabled)
	if err != nil {
...
func (db *database) setSettings(s Settings) error {
	...
	_, err := db.conn.Exec(`
		UPDATE settings
		SET expose = ?, survey = ?, browser = ?, models = ?, agent = ?, tools = ?, working_dir = ?, context_length = ?, turbo_enabled = ?, websearch_enabled = ?, selected_model = ?, sidebar_open = ?, last_home_view = ?, start_minimized = ?, think_enabled = ?, think_level = ?, auto_update_enabled = ?
	`, s.Expose, s.Survey, s.Browser, s.Models, s.Agent, s.Tools, s.WorkingDir, s.ContextLength, s.TurboEnabled, s.WebSearchEnabled, s.SelectedModel, s.SidebarOpen, lastHomeView, s.StartMinimized, s.ThinkEnabled, s.ThinkLevel, s.AutoUpdateEnabled)
	if err != nil {
