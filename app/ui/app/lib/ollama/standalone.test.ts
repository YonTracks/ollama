...
describe("standalone Ollama client", () => {
  ...
  it("shows model details from the core /api/show endpoint with full Modelfile fields", async () => {
    const fetchMock = vi.fn((url: string, init?: RequestInit) => {
      void url;
      void init;
      return Promise.resolve(
    ...
    await expect(showStandaloneModel("llama3.2", undefined, "test-token", undefined, {
      verbose: true
    })).resolves.toMatchObject({
      modelfile: expect.stringContaining("FROM llama3.2"),
      capabilities: ["completion"]
    });
    expect(fetchMock).toHaveBeenCalledWith(
      `${DEFAULT_CORE_API_BASE}/api/show`,
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ model: "llama3.2", verbose: true })
      })
    );
    expect(authorizationHeader(fetchMock.mock.calls[0]?.[1])).toBe("Bearer test-token");
  });
  ...
  it("creates models with structured advanced options", async () => {
    const createStream = new ReadableStream<Uint8Array>({
      start(controller) {
        controller.enqueue(new TextEncoder().encode('{"status":"success"}\n'));
        controller.close();
    ...
    const fetchMock = vi.fn(() => Promise.resolve(new Response(createStream, { status: 200 })));
    vi.stubGlobal("fetch", fetchMock);
